...
type PProf struct {
	...
	EnableProfiles *ProfilesConfig `mapstructure:"enable_profiles" yaml:"enable_profiles" json:"enable_profiles"` // 启用的性能分析
	...
	WebInterface   *WebConfig      `mapstructure:"web_interface" yaml:"web_interface" json:"web_interface"`       // Web界面配置
}

// AuthConfig 认证配置
type AuthConfig struct {
	Enabled     bool     `mapstructure:"enabled" yaml:"enabled" json:"enabled"`                // 是否启用认证
	AuthToken   string   `mapstructure:"auth_token" yaml:"auth_token" json:"auth_token"`       // 认证令牌
	AllowedIPs  []string `mapstructure:"allowed_ips" yaml:"allowed_ips" json:"allowed_ips"`    // 允许的IP列表
	RequireAuth bool     `mapstructure:"require_auth" yaml:"require_auth" json:"require_auth"` // 是否需要认证
...
type GatewayConfig struct {
	...
	DevModeOnly          bool `mapstructure:"dev_mode_only" yaml:"dev_mode_only" json:"dev_mode_only"`                            // 仅在开发模式启用
	EnableLogging        bool `mapstructure:"enable_logging" yaml:"enable_logging" json:"enable_logging"`                         // 是否启用日志
	RegisterWebInterface bool `mapstructure:"register_web_interface" yaml:"register_web_interface" json:"register_web_interface"` // 是否注册Web界面
}
...
type WebConfig struct {
	...
	ShowScenarios bool   `mapstructure:"show_scenarios" yaml:"show_scenarios" json:"show_scenarios"` // 是否显示性能测试场景
}
...
type SamplingConfig struct {
	CPURate       int `mapstructure:"cpu_rate" yaml:"cpu_rate" json:"cpu_rate"`                   // CPU采样率(Hz)
	MemoryRate    int `mapstructure:"memory_rate" yaml:"memory_rate" json:"memory_rate"`          // 内存采样率
	BlockRate     int `mapstructure:"block_rate" yaml:"block_rate" json:"block_rate"`             // 阻塞采样率
	MutexFraction int `mapstructure:"mutex_fraction" yaml:"mutex_fraction" json:"mutex_fraction"` // 互斥锁采样比例
}
