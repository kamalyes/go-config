...
type JWT struct {
	...
	SigningKey       string                 `mapstructure:"signing_key" yaml:"signing-key" json:"signing_key"      validate:"required"`              // jwt 签名
	ExpiresTime      int64                  `mapstructure:"expires_time" yaml:"expires-time" json:"expires_time"     validate:"required,min=1"`      // 过期时间（单位：秒）
	BufferTime       int64                  `mapstructure:"buffer_time" yaml:"buffer-time" json:"buffer_time"      validate:"required,min=0"`        // 缓冲时间（单位：秒）
	UseMultipoint    bool                   `mapstructure:"use_multipoint" yaml:"use-multipoint" json:"use_multipoint"`                              // 多地登录拦截，true 表示拦截，false 表示不拦截
	...
	EnableRefresh    bool                   `mapstructure:"enable_refresh" yaml:"enable-refresh" json:"enable_refresh"`                              // 是否启用刷新 token
	RefreshTokenLife int64                  `mapstructure:"refresh_token_life" yaml:"refresh-token-life" json:"refresh_token_life" validate:"min=1"` // 刷新 token 生命周期（单位：秒）
	Subject          string                 `mapstructure:"subject" yaml:"subject" json:"subject"`                                                   // JWT 主题
	CustomClaims     map[string]interface{} `mapstructure:"custom_claims" yaml:"custom-claims" json:"custom_claims"`                                 // 自定义声明
}
