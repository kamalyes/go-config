...
type Ristretto struct {
	...
	NumCounters        int64  `mapstructure:"num-counters" yaml:"num-counters" json:"numCounters" validate:"min=1"`       // 计数器数量，用于跟踪访问频率
	MaxCost            int64  `mapstructure:"max-cost" yaml:"max-cost" json:"maxCost" validate:"min=1"`                   // 最大缓存成本
	BufferItems        int64  `mapstructure:"buffer-items" yaml:"buffer-items" json:"bufferItems" validate:"min=1"`       // Get 缓存的大小
	Metrics            bool   `mapstructure:"metrics" yaml:"metrics" json:"metrics"`                                      // 是否启用缓存统计
	IgnoreInternalCost bool   `mapstructure:"ignore-internal-cost" yaml:"ignore-internal-cost" json:"ignoreInternalCost"` // 是否忽略内部成本计算
	KeyToHash          bool   `mapstructure:"key-to-hash" yaml:"key-to-hash" json:"keyToHash"`                            // 是否将键转换为哈希
