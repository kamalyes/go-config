...
type Kafka struct {
	...
	GroupID     string `mapstructure:"group_id" yaml:"group-id" json:"group_id"`                                        // 消费者组ID
	...
	TryTimes    int    `mapstructure:"try_times" yaml:"try-times" json:"try_times"         validate:"required,min=1"`   // 尝试次数
	SyncESTopic string `mapstructure:"sync_es_topic" yaml:"sync-es-topic" json:"sync_es_topic"     validate:"required"` // 同步到 ES 的主题
