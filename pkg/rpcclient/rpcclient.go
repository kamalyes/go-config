...
type RpcClient struct {
	...
	NonBlock       bool              `mapstructure:"non-block" yaml:"non-block" json:"nonBlock"`                   // 是否非阻塞
	...
	DialTimeout    int               `mapstructure:"dial-timeout" yaml:"dial-timeout" json:"dialTimeout"`          // 连接超时(秒)
	MaxRetries     int               `mapstructure:"max-retries" yaml:"max-retries" json:"maxRetries"`             // 最大重试次数
	RetryInterval  int               `mapstructure:"retry-interval" yaml:"retry-interval" json:"retryInterval"`    // 重试间隔(毫秒)
	LoadBalance    string            `mapstructure:"load-balance" yaml:"load-balance" json:"loadBalance"`          // 负载均衡策略 (round_robin, random, weighted)
	...
	CircuitBreaker *CircuitBreaker   `mapstructure:"circuit-breaker" yaml:"circuit-breaker" json:"circuitBreaker"` // 熔断器配置
}
...
type TLSConfig struct {
	...
	ServerName         string `mapstructure:"server-name" yaml:"server-name" json:"serverName"`                           // 服务器名称
	InsecureSkipVerify bool   `mapstructure:"insecure-skip-verify" yaml:"insecure-skip-verify" json:"insecureSkipVerify"` // 跳过证书验证
}

// CircuitBreaker 熔断器配置
type CircuitBreaker struct {
	Enabled          bool `mapstructure:"enabled" yaml:"enabled" json:"enabled"`                                  // 是否启用熔断器
	MaxRequests      int  `mapstructure:"max-requests" yaml:"max-requests" json:"maxRequests"`                    // 最大请求数
	...
	HalfOpenMaxCalls int  `mapstructure:"half-open-max-calls" yaml:"half-open-max-calls" json:"halfOpenMaxCalls"` // 半开状态最大调用数
}
