...
func TestDatabase_NewDatabaseIncludesClickHouseAndCockroachDB(t *testing.T) {
	db := NewDatabase()
	assert.NotNil(t, db.ClickHouse)
	assert.NotNil(t, db.CockroachDB)
...
func TestDatabase_EnableClickHouseAndCockroachDB(t *testing.T) {
	...
	db.EnableClickHouse()
	assert.Equal(t, DBTypeClickHouse, db.Type)
	assert.Equal(t, string(DBTypeClickHouse), db.Default)

	db.EnableCockroachDB()
	assert.Equal(t, DBTypeCockroachDB, db.Type)
	assert.Equal(t, string(DBTypeCockroachDB), db.Default)
}
