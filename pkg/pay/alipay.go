...
type AliPay struct {
	...
	PriKey     string `mapstructure:"pri_key" yaml:"pri-key" json:"pri_key"         validate:"required"`                    // 私钥
	PubKey     string `mapstructure:"pub_key" yaml:"pub-key" json:"pub_key"         validate:"required"`                    // 公钥，主要是回调验签用
	SignType   string `mapstructure:"sign_type" yaml:"sign-type" json:"sign_type"       validate:"required,oneof=RSA2 RSA"` // 签名方式，支持 RSA2 和 RSA
	NotifyUrl  string `mapstructure:"notify_url" yaml:"notify-url" json:"notify_url"      validate:"required,url"`          // 支付宝回调的 URL
