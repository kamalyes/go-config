...
type Zap struct {
	...
	MaxSize       int    `mapstructure:"max_size" yaml:"max-size" json:"max_size"              validate:"required,min=1"`                       // 日志文件的最大大小（以MB为单位）
	MaxAge        int    `mapstructure:"max_age" yaml:"max-age" json:"max_age"               validate:"required,min=1"`                         // 日志最大保留时间 单位：天
	MaxBackups    int    `mapstructure:"max_backups" yaml:"max-backups" json:"max_backups"           validate:"required,min=0"`                 // 保留旧文件的最大个数
	Compress      bool   `mapstructure:"compress" yaml:"compress" json:"compress"`                                                              // 是否压缩
	LinkName      string `mapstructure:"link_name" yaml:"link-name" json:"link_name"`                                                           // 日志软连接文件
	ShowLine      bool   `mapstructure:"show_line" yaml:"show-line" json:"show_line"`                                                           // 是否在日志中输出源码所在的行
	EncodeLevel   string `mapstructure:"encode_level" yaml:"encode-level" json:"encode_level"`                                                  // 日志编码等级，指定不通过等级可以有不同颜色
	StacktraceKey string `mapstructure:"stacktrace_key" yaml:"stacktrace-key" json:"stacktrace_key"`                                            // 堆栈捕捉标识
	LogInConsole  bool   `mapstructure:"log_in_console" yaml:"log-in-console" json:"log_in_console"`                                            // 是否在控制台打印日志
