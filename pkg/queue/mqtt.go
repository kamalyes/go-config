...
type Mqtt struct {
	...
	ClientID             string `mapstructure:"client-id"                yaml:"client_id"       json:"client_id"`                                      // 客户端标识符
	ProtocolVersion      uint   `mapstructure:"protocol-version"         yaml:"protocol_ver"    json:"protocol_version" validate:"required"`           // Mqtt 协议版本号，4 是 3.1.1，3 是 3.1
	KeepAlive            int    `mapstructure:"keep-alive"               yaml:"keep_alive"      json:"keep_alive"      validate:"required,min=1"`      // 保活时间间隔，最小值为 1 秒
	MaxReconnectInterval int    `mapstructure:"max-reconnect-interval"   yaml:"max_reconnect_interval" json:"max_reconnect_interval" validate:"min=1"` // 最大连接间隔时间，单位：秒，最小值为 1 秒
	PingTimeout          int    `mapstructure:"ping-timeout"             yaml:"ping_timeout"    json:"ping_timeout"      validate:"min=1"`             // ping 超时时间，单位：秒，最小值为 1 秒
	WriteTimeout         int    `mapstructure:"write-timeout"            yaml:"write_timeout"   json:"write_timeout"     validate:"min=1"`             // 写超时时间，单位：秒，最小值为 1 秒
	ConnectTimeout       int    `mapstructure:"connect-timeout"          yaml:"connect_timeout" json:"connect_timeout"   validate:"min=1"`             // 连接超时时间，单位：秒，最小值为 1 秒
	...
	CleanSession         bool   `mapstructure:"clean-session"            yaml:"clean_session"   json:"clean_session"`                                  // 设置客户端掉线服务端是否清除 session
	AutoReconnect        bool   `mapstructure:"auto-reconnect"           yaml:"auto_reconnect"  json:"auto_reconnect"`                                 // 断开后是否重新连接
	WillTopic            string `mapstructure:"will-topic"               yaml:"will_topic"      json:"will_topic"`                                     // 遗言发送的 topic
