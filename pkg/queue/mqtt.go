...
type Mqtt struct {
	...
	ClientID             string `mapstructure:"client_id" yaml:"client-id" json:"client_id"`                                                         // 客户端标识符
	ProtocolVersion      uint   `mapstructure:"protocol_version" yaml:"protocol-version" json:"protocol_version" validate:"required"`                // Mqtt 协议版本号，4 是 3.1.1，3 是 3.1
	KeepAlive            int    `mapstructure:"keep_alive" yaml:"keep-alive" json:"keep_alive"      validate:"required,min=1"`                       // 保活时间间隔，最小值为 1 秒
	MaxReconnectInterval int    `mapstructure:"max_reconnect_interval" yaml:"max-reconnect-interval" json:"max_reconnect_interval" validate:"min=1"` // 最大连接间隔时间，单位：秒，最小值为 1 秒
	PingTimeout          int    `mapstructure:"ping_timeout" yaml:"ping-timeout" json:"ping_timeout"      validate:"min=1"`                          // ping 超时时间，单位：秒，最小值为 1 秒
	WriteTimeout         int    `mapstructure:"write_timeout" yaml:"write-timeout" json:"write_timeout"     validate:"min=1"`                        // 写超时时间，单位：秒，最小值为 1 秒
	ConnectTimeout       int    `mapstructure:"connect_timeout" yaml:"connect-timeout" json:"connect_timeout"   validate:"min=1"`                    // 连接超时时间，单位：秒，最小值为 1 秒
	...
	CleanSession         bool   `mapstructure:"clean_session" yaml:"clean-session" json:"clean_session"`                                             // 设置客户端掉线服务端是否清除 session
	AutoReconnect        bool   `mapstructure:"auto_reconnect" yaml:"auto-reconnect" json:"auto_reconnect"`                                          // 断开后是否重新连接
	WillTopic            string `mapstructure:"will_topic" yaml:"will-topic" json:"will_topic"`                                                      // 遗言发送的 topic
