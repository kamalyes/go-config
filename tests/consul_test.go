 ...
 * @LastEditTime: 2025-11-11 10:32:20
...
// 生成随机的 Consul 配置参数
func generateConsulTestParams() *consul.Consul {
...
func assertConsulFields(t *testing.T, consul *consul.Consul, expected *consul.Consul) {
...
func TestNewConsul(t *testing.T) {
	...
	consulInstance := consul.NewConsul(params)
...
func TestConsulClone(t *testing.T) {
	params := generateConsulTestParams()
	original := consul.NewConsul(params)
	cloned := original.Clone().(*consul.Consul)
...
func TestConsulSet(t *testing.T) {
	...
	newParams := &consul.Consul{
		ModuleName:       "UpdatedModule",
		Endpoint:         "192.168.1.1:8500",
		RegisterInterval: 15,
	}

	consulInstance := consul.NewConsul(oldParams)
