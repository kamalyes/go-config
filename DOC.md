...
| **缓存系统** | Cache | 多种缓存实现：Memory、Ristretto、Redis、Sharded、TwoLevel、Expiring |
| **数据存储** | MySQL, PostgreSQL, SQLite, Redis | 关系型数据库和缓存 |
| **对象存储** | Minio, AliyunOSS, S3, FTP | 文件存储服务 |
| **消息队列** | MQTT, Kafka | 消息中间件 |
| **监控日志** | Zap, Jaeger, Elasticsearch, PProf | 日志、链路追踪和性能分析 |
| **第三方API** | 支付宝、微信支付、阿里云短信、阿里云STS | 第三方服务集成 |
| **服务治理** | Consul, Zero(go-zero) | 服务发现和微服务框架 |
| **安全认证** | Captcha | 验证码生成配置 |
| **通信服务** | Email | 邮件发送服务 |
| **电商平台** | YouZan | 有赞电商平台接口 |
...
func main() {
    ...
    // 使用配置
    log.Printf("服务地址: %s", config.Server.Endpoint)
...
`go-config` 提供了统一的缓存配置接口，支持多种缓存实现：

```go
type Cache struct {
    ModuleName string    `yaml:"module_name"`
    Type       CacheType `yaml:"type"`         // memory, ristretto, redis, sharded, two-level, expiring
...
cache:
  ...
  memory:
    capacity: 1000                    # 缓存容量
    default_ttl: "30m"               # 默认过期时间
    cleanup_size: 100                # 清理大小
    max_size: 0                      # 最大大小(0=无限制)
  
  # Ristretto 高性能缓存配置
  ristretto:
    num_counters: 100000             # 计数器数量
    max_cost: 1000000                # 最大成本
    buffer_items: 64                 # 缓冲项目数
  ...
  sharded:
    shard_count: 32                  # 分片数量
    max_entries_per_shard: 1000      # 每个分片最大条目
...
type PProf struct {
    ...
    PathPrefix     string   `yaml:"path_prefix"`     // pprof 路径前缀
    AllowedIPs     []string `yaml:"allowed_ips"`     // 允许访问的 IP 地址列表
    RequireAuth    bool     `yaml:"require_auth"`    // 是否需要认证
    AuthToken      string   `yaml:"auth_token"`      // 认证令牌
    EnableLogging  bool     `yaml:"enable_logging"`  // 是否启用请求日志
    Timeout        int      `yaml:"timeout"`         // 超时时间（秒）
    ModuleName     string   `yaml:"modulename"`      // 模块名称
}
```
...
**安全建议:**
- 生产环境建议设置 `enabled: false`
- 如果必须启用，请配置 `allowed_ips` 限制访问
- 敏感环境建议启用 `require_auth` 并设置强密码
- 使用 `enable_logging: true` 监控访问记录
...
`go-config` v1.1+ 版本新增了对外部 Viper 实例的支持，让您可以管理任意的配置源：
...
func externalViperExample() {
    ...
    customViper.SetConfigFile("./custom_config.yaml")
    err = customViper.ReadInConfig()
    if err != nil {
        log.Printf("Warning: Failed to read custom config: %v", err)
    }
    
    // 添加外部 Viper 实例
    config.AddExternalViper("custom", customViper)
    
    // 从外部 Viper 获取配置值
    if viper, exists := config.GetExternalViper("custom"); exists {
        customValue := viper.GetString("custom.setting")
        log.Printf("Custom setting: %s", customValue)
    }
    ...
    if err != nil {
        log.Printf("Error unmarshaling custom config: %v", err)
    } else {
        log.Printf("Custom Config: %+v", custom)
    }
    
    // 获取所有外部 Viper 键
    keys := config.GetAllExternalViperKeys()
    log.Printf("External Viper keys: %v", keys)
    
    // 移除外部 Viper 实例
    removed := config.RemoveExternalViper("custom")
    log.Printf("Removed custom viper: %t", removed)
}
```

### 2. 动态配置管理

支持运行时动态添加和管理配置，无需重启应用：
...
func dynamicConfigExample() {
    ...
    // 设置运行时参数
    config.SetDynamicConfig("runtime_params", map[string]interface{}{
    ...
    if flags, exists := config.GetDynamicConfig("feature_flags"); exists {
        ...
        if featureFlags["new_api_v2"] {
            log.Println("✅ 启用新 API v2")
        }
        if !featureFlags["legacy_support"] {
            log.Println("❌ 禁用遗留支持")
        }
    }
    
    // 获取运行时参数
    if params, exists := config.GetDynamicConfig("runtime_params"); exists {
        runtimeParams := params.(map[string]interface{})
        maxConn := runtimeParams["max_connections"].(int)
        timeout := runtimeParams["timeout_seconds"].(int)
        log.Printf("📊 运行时参数: 最大连接数=%d, 超时=%ds", maxConn, timeout)
    }
    
    // 获取所有动态配置键
    keys := config.GetAllDynamicConfigKeys()
    log.Printf("📋 动态配置键: %v", keys)
    
    // 移除动态配置
    removed := config.RemoveDynamicConfig("runtime_params")
    log.Printf("🗑️ 移除运行时参数: %t", removed)
}
```
